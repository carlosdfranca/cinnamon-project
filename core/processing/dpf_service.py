    ...
    def _montar_secao(tipo: int, label_total: str) -> Tuple[Dict[str, Dict], int, int]:
        ...
        for grupao in grupoes:
            ...
            for grupinho in sorted(grupao.grupinhos.all(), key=lambda g: g.nome):
                atual = _int_mil(somas.get((tipo, grupao.id, grupinho.id, data_atual), 0.0), dividir_por_mil)
                anterior = _int_mil(somas.get((tipo, grupao.id, grupinho.id, data_anterior), 0.0), dividir_por_mil)
